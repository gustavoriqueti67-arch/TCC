...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($nome_foto) {
            ...
            if (!move_uploaded_file($foto_perfil['tmp_name'], UPLOADS_PATH . $nome_foto)) {
        ...
        if(empty($errors)) {
            // --- CORREÇÃO FINAL PARA PHP ANTIGO (SEM OpenSSL) ---
            // Gerar um "salt" aleatório sem depender da extensão OpenSSL, usando mt_rand().
...
<head>
    ...
    <style>
        ...
        .form-group-photo label:hover .profile-pic-preview { border-color: var(--primary-blue); filter: brightness(0.8); }
