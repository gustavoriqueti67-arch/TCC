...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if (file_put_contents($upload_dir . $nome_foto, $decoded_image) === false) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO animais (nome, especie, raca, idade, sexo, descricao, foto_animal, id_usuario, telefone_contato, endereco, cidade, data_cadastro) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
                $stmt->execute([
                    $nome, 
                    $especie, 
                    $raca, 
                ...
                $success_message = "Animal cadastrado com sucesso! Redirecionando...";
